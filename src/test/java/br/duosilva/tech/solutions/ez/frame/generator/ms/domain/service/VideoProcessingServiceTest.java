...
class VideoProcessingServiceTest {
    ...
    @Test
    void testGenerateVideoFrames() throws Exception {
        // Arrange
        File videoFile = new File("test-video.mp4");

        // Create mock frame files and their parent directory
        File frame1 = mock(File.class);
        File frame2 = mock(File.class);
        ...
        when(ffmpegFrameExtractor.extractFramesFromVideo(videoFile)).thenReturn(frames);

        File zipFile = new File("frames.zip");
        when(zipFileGenerator.generateZipFromFrames(frames, "test-video")).thenReturn(zipFile);

        // Act
        File result = videoProcessingService.generateVideoFrames(videoFile);
        ...
        verify(frame1).delete();
        verify(frame2).delete();
        verify(parentDirectory).delete(); // Verify that the parent directory is deleted
    }

    @Test
    void testGenerateFrames() throws Exception {
        // Arrange
