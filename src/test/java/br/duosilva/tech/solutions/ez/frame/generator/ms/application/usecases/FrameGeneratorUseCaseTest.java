...
class FrameGeneratorUseCaseTest {
    ...
    void shouldProcessVideoSuccessfully() throws Exception {
        // given
        VideoDataResponseDto dto = new VideoDataResponseDto("video_1", "video.mp4", "bucket", "key", "12:00", "user_1", "test@test.com");
        ...
        mockStatic(FileUtils.class).when(() -> FileUtils.convertStreamToFile(any(), eq(".mp4")))
        ...
        // when
        useCase.retrieveAndProcessBucketVideo(dto);
    ...
    void shouldThrowBusinessRuleExceptionOnFailure() throws Exception {
        ...
        // when & then
        assertThrows(BusinessRuleException.class, () -> useCase.retrieveAndProcessBucketVideo(dto));
    }
}
